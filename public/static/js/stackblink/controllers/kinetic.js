function KineticCtrl($scope, $http) {
  ...
  var CONTROL_URL = "/asterank/api/stackblink/get_neat_control_group";
  var UNKNOWN_URL = "/asterank/api/stackblink/get_sdss_unknown_group";
  ...
  function UserResponse(interesting, poor_quality) {
    ...
    $http
      .post("/asterank/api/stackblink/record", {
        email: $scope.email,
        ...
        circle_coords: (function () {
          ...
          angular.forEach(circles, function (circle_layer) {
            coords.push([
              circle_layer.children[0].getX(),
              circle_layer.children[0].getY(),
            ]);
          });
          return coords;
        })(),
  ...
  $scope.PromptForEmail = function () {
    $scope.email = prompt(
      "Please enter your email address so we can associate any potential discoveries with your name.\n\nYour email will not be used for any other purposes."
    );
  };
}
