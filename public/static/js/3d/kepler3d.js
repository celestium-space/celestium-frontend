(function () {
  ...
  $("#webgl_container").height(
    $(window).height() - $("#webgl_container").offset().top - 20
  );
  ...
  asterank3d = new Asterank3D({
    ...
    custom_object_fn: function (obj) {
      ...
      if (
        ...
        heatcolor = new THREE.Color(
          getColorFromPercent(pct_temp, 0xff0000, 0x0000ff)
        );
      }
    ...
    object_texture_path: "/asterank/static/img/cloud_defined.png",
  ...
  $.getJSON(
    '/asterank/api/exoplanets?query={"a":{"$ne":"", "$gt": 0.1}}&limit=3000',
    function (data) {
      $.each(data, function () {
