...
class LogicHandler {
    constructor(grid, pixelControls, store) {
        ...
        this.getSocket().then(socket => socket.send(Uint8Array.from([CMDOpcodes.GET_ENTIRE_IMAGE])));
    }
    ...
    async clickPixel(x, y, rgb) {
        ...
        this.getSocket().then(socket => {
            ...
            let to_send = Uint8Array.from([CMDOpcodes.GET_PIXEL_DATA, x >> 8, x & 0xff, y >> 8, y & 0xff]);
        ...
        switch (cmdOpcode) {
            case CMDOpcodes.ENTIRE_IMAGE:
                if (this.grid) {
                    ...
                    let color = array.slice(1).map(c => intToColor(c)).flat();
            ...
            case CMDOpcodes.UPDATE_PIXEL:
                if (this.grid) {
                    ...
                    let color = array.slice(5).map(c => intToColor(c)).flat();
            ...
            case CMDOpcodes.PIXEL_DATA:
                if (this.mining_data) {
                    ...
                    let transaction = await generateAndMinePixelNFT(x, y, index, block_hash, pixel_hash);
                ...
                break;
            default:
                console.warn(`WS Server sent unknown or unexpected command "${cmdOpcode}", ignoring`);
                break;
        }
    }
